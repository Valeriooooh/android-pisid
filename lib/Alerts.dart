...
class Mensagens extends StatelessWidget {//sets up the main structure for the 'Mensagens' screen
//class Mensagens extends StatefulWidget {
  const Mensagens({super.key});
  @override
  Widget build(BuildContext context) { //builds the UI for the 'Mensagens' screen. 
    const appTitle = 'Mensagens';

    return Scaffold( //Uses a 'scaffold' widget to provide a basic material design layout structure, including an 'AppBar' with the title 'Mensagens' and the main body containing the 'MensagensMain' widget.
      appBar: AppBar(
        centerTitle: true,
...
class MensagensMain extends StatefulWidget {  //represents the main content area of the 'Mensagens' screen.
  const MensagensMain({super.key});  
  @override

  MensagensMainState createState() { //creates and returns an instance of 'MensagensMainState', which will hold the mutable state for 'MensagensMain'
    return MensagensMainState();
  }
}

class MensagensMainState extends State<MensagensMain> {  //State object for the 'MensagensMain' widget
  ...
  Future<void> _onItemTapped(int index) async {  //Method called when a navigation item is tapped
    setState(() {
  ...
  @override
  void initState() {  //Initializes the state of the widget and sets up a periodic timer to fetch messages
    const oneSec = Duration(seconds:1);  //Interval for the timer
    timer = Timer.periodic(oneSec, (Timer t) => getMensagens());  //Periodically fetch messages
    super.initState();
  }
  ...
  selectDate(BuildContext context) async {  //Method to select a date using a date picker
        ...
        setState(() {
          ...
          for (var i = 0; i < Mensagens.length; i++) {
            ...
            int timeKey = int.parse(
                newMensagens["Hora"].toString().split(" ")[1].replaceAll(":", ""));
  ...
  listMensagens() {  //Method to generate data rows for the data table
    ...
    for (var i = tableMensagens.length - 1; i >= 0; i--) {
      ...
      tableMensagens[key]?.forEach((MensagensField) {
        if (i == 0) {
          MensagensRow.add(DataCell(Text(MensagensField, style: const TextStyle(color: Colors.blue))));
        } else {
    ...
    mostRecentMensagens = tableMensagens.keys.elementAt(tableMensagens.length-1);
    return MensagensList.reversed.toList();
  }

  listFields() {  //Method to generate data columns for the data table
    var fields = <DataColumn>[];
    for (var field in tableFields) {
      fields.add(DataColumn(label: Text(field)));  //Create a DataColumn for each field
    }
    return fields;
  }
