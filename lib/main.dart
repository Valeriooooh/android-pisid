...
class _LoginPageState extends State<LoginPage> {
  ...
  final usernameController = TextEditingController(
    text: "root",
  ); //controller that manages input for the username
  final passwordController = TextEditingController(
    text: "",
  ); //controller that manages the input for the password
  final ipController = TextEditingController(
    text: "127.0.0.1",
  ); //controller that manages input for the IP address
  final portController = TextEditingController(
    text: "80",
  ); //controller that manages input for the port
  ...
  validateLogin() async {
    String loginURL =
        "http://${ipController.text.trim()}:${portController.text.trim()}/scripts/php/validateLogin.php";
    print(loginURL);
    ...
    if (response.statusCode == 200) {
      ...
      if (jsonData["success"]) {
        ...
        showDialog(
          context: context,
          builder: (context) {
            return const AlertDialog(content: Text("Mensagens"));
          },
        );
      }
    }
